...
def runmodels_returnresults(label_name,
    ...
    for i in range(0, len(model_list)):
        ...
        ## before inserting, check if model group values already exists
...
#             ### pass value to schematable_insert for sql_insert_function
            ...
            modelexists_result = 'continue_fitting'
        ...
        ## fit model
        one_model.fit(gaussianKernelGramMatrixFull(x_train_features.values,x_train_features.values), label_train_mat)
...
#          ## create values to insert for feature_importance table
#         feature_imp_row_list = []
#         for index,row in featureimportances_df.iterrows():
#             feature_imp_row_list.append(', '.join(map(lambda x: "'" + x + "'", list(row.astype(str)))))
#             feature_imp_value_list= '), ('.join(feature_imp_row_list)

#         print('created values_to_insert for feature importances table')



#          ### get columns to insert for feature_importance table
# #         ### pass value to table for get_table_col function
#         table = 'feature_importances'
#         feature_importances_columns_to_insert = get_table_col(creds,table,get_cols_intable_redshift=get_cols_intable_redshift)

#         ### pass value to schematable_insert for sql_insert_function
#         feature_importances_schematable_insert = 'dssg_results.feature_importances'
#         sql_insert_function(feature_importances_schematable_insert,feature_importances_columns_to_insert,
#                              feature_imp_value_list,creds)
#         print("updated row for feature_importances table")
        ...
        for data, group in df_test_grouped:
            ...
            label_prob = one_model.predict_proba(gaussianKernelGramMatrixFull(x_test_mat, x_train_features.values))
